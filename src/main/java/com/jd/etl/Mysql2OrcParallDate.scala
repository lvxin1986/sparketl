...
object Mysql2OrcParallDate {
    ...
    def ImportDataDaily(confPath: String, filterCol: String, filterVal1: String, filterVal2: String): Unit = {
        ...
        for (i <- 0 until tblsArray.length) {
            var tbSch: Array[String] = tblsArray.apply(i).split("\\s+")
            //var tableName = tbSch.apply(0)
            var tableName = "product_with_1_partition"
            var columns = schemaMap.apply(tbSch.apply(1))
            var sql:String = ""
            if (columns.equals("") || columns == null) {
                sql = "select * from " + tbSch.apply(0)
            ...
            if (filterCol != null && !filterCol.equals("") && filterVal1 != null && !filterVal1.equals("") && filterVal2 != null && !filterVal2.equals("")) {
                ...
                if (columns.length == values1.length) {
                    for (j <- 0 until columns.length) {
                        ...
                        if (j == 0) {
                            ...
                            sql = sql + " or " + col_name + ">"+quotation + values1.apply(j) + " 00:00:00"+quotation + " and " + col_name + "<"+quotation + values2.apply(j) + " 00:00:00"+quotation
                        }
                    ...
                    var message = "[INFO] execute the sql : " + sql + " and store in memory and disk"

                    for (j <- 0 until columns.length){
                        //var col_name = columns.apply(j)
                        var start = values1.apply(j) + " 00:00:00"
                        var end = values2.apply(j) + " 00:00:00"
                        val c_t = columns.apply(j).split(":")
                        val col_name=c_t.apply(0)
                        val col_type = c_t.apply(1)
                        var quotation = "";
                        ...
                        while (!start.equals(end)) {
                            ...
                            //var filterSql = sql + " where " + col_name + ">"+quotation + yesterday +quotation + " and " + col_name + "<"+quotation + end +quotation

                            //partitionPath = partitionPath + "/" + col_name + "=" + yesterday.substring(0,10)
                            partitionPath = "/dt=" + yesterday.substring(0,10) + "/"
                            var condition = col_name + ">" +quotation + yesterday+ quotation + " and " + col_name + "<" + quotation+ end + quotation

                            var totalPath = orcPath + tableName + partitionPath
                            ...
                            //val task = doEtlPerTbl(spark, filterSql, totalPath, orcCoalesce)
                ...
                if (dirExists(orcPath + tableName + partitionPath + "/_SUCCESS")) {
                    ...
                    val task = doEtlPerTbl(df, null, orcPath + tableName + partitionPath, orcCoalesce)
                    task.onComplete {
                        case Success(result) =>
                            println(s"result = $result")
                            status += result
