...
object Mysql2OrcParall {
    ...
    def main(args: Array[String]) {
        var confPath = args(0)
        var filterCol = args(1)
        var filterVal1 = args(2)
        var filterVal2 = args(3)
...
//        spark.conf.set("spark.executor.memory", "2g")
        ...
        var orcCoalesce = properties.getProperty("etl.mysql2orc.orc.coalesce").toInt
        ...
        for(i <- 0 until tblsArray.length){
            ...
            /*if (filterCol != null && !filterCol.equals("") && filterVal1 != null && !filterVal1.equals("") && filterVal2 != null && !filterVal2.equals("")) {
                var columns : Array[String] = filterCol.split(",")
                var values1 : Array[String] = filterVal1.split(",")
                var values2 : Array[String] = filterVal2.split(",")
                if (columns.length == values1.length) {
                    for (j <- 0 until columns.length) {
                        /*if (j == 0){
                            sql = sql + " where " + columns.apply(j) + ">'" + values1.apply(j) + " 00:00:00'" + " and " + columns.apply(j) + "<'" + values2.apply(j) + " 00:00:00'"
                        } else {
                            sql = sql + " or " + columns.apply(j) + ">'" + values1.apply(j) + " 00:00:00'" + " and " + columns.apply(j) + "<'" + values2.apply(j) + " 00:00:00'"
                        }*/
//                        partitionPath = partitionPath +"/"+columns.apply(j)+"="+ values1.apply(j)
                    }
                } else {
                    println("[ERROR]columns length is not eequal values length")
                }
            }*/
